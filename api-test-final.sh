...
# Lab Operations API Test Suite - Final Version
# This script tests all API endpoints and generates comprehensive reports
...
test_api() {
    ...
    if [ "$status_code" = "$expected_status" ]; then
        ...
        TEST_RESULTS+=("$TOTAL_TESTS|PASS|$description|$method|$endpoint|$status_code|$body")
    else
        log "${RED}❌ FAILED${NC} - Expected: $expected_status, Got: $status_code"
        FAILED_TESTS=$((FAILED_TESTS + 1))
        TEST_RESULTS+=("$TOTAL_TESTS|FAIL|$description|$method|$endpoint|$status_code|$body")
    fi
...
run_tests() {
    ...
    template_response=$(test_api "POST" "/test-templates" '{"name": "Complete Blood Count", "description": "Full blood analysis", "basePrice": 500.00, "parameters": {"hemoglobin": {"type": "number", "unit": "g/dL", "normalRange": "12-16"}}}' "201" "Create Test Template")
...
EOF
    ...
    for result in "${TEST_RESULTS[@]}"; do
        IFS='|' read -r test_num status description method endpoint status_code response <<< "$result"
